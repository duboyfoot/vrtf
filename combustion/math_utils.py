"""
Fonctions mathématiques : interpolation, polynômes, bisection.
"""
...
def poly_cp(coeffs: list, T: float) -> float:
    """
    Évalue Cp(T) = a + b*T + c*T² + ... (jusqu'à 7 termes).
...
def poly_enthalpy(coeffs: list, T: float, T_ref: float = 273.0) -> float:
    """
    Intègre Cp(T) de T_ref à T : h = Σ c_j/j * (T^j - T_ref^j).
    Retourne [J/kg].
    """
    h = 0.0
...
def poly_integrate(coeffs: list, x1: float, x2: float) -> float:
    """Intégrale d'un polynôme (coeffs = [a0, a1, ...]) de x1 à x2."""
    result = 0.0
    for i, c in enumerate(coeffs, start=1):
        if c:
